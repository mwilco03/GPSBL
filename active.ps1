...
function Activate-Win {
    ...
    $commands = @("-upk", "-ipk W269N-WFGWX-YVC9B-4J6C9-T83GX", "-skms $hstName", "-ato")
    foreach ($cmd in $commands) {
        ...
        switch ($cmd.split()[0]) {
            ...
            "-ato" { Write-Host "Activation Attempted" -ForegroundColor Yellow }
        }
        Start-Sleep 5
    }

    # Check if the current Windows version matches the valid activated ones
    $sku = (Get-CimInstance -ClassName Win32_OperatingSystem).OperatingSystemSKU
    if ($sku -in 4, 48, 96, 98, 121, 125) {
